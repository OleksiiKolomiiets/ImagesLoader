...
extension ImagesViewController: UIDropInteractionDelegate, UICollectionViewDropDelegate, UITableViewDragDelegate {
    ...
    private func setUpFavoriteImagesViewController(with droppedItems: [NSItemProviderReading], in tabBarViewControllers: [UIViewController]) {
        ...
        for data in dataArray {
            if CoreDataFavoriteManager.isOverlapTheLimit {
                let alert = UIAlertController(title: "Favorite Image Alert", message: "Favorite images limit(\(CoreDataFavoriteManager.maxImagesCount)) is overlaped. Please click 'OK' and delete some.", preferredStyle: .alert)
                ...
                CoreDataFavoriteManager.addFavoriteImage(ImageData.instance(from: data))
            }
        }
    }
